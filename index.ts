...
setInterval(() => {
  processStripeWebhookRetryQueue(stripeWebhookRetryHandler)
    .catch(e => console.error('[STRIPE WEBHOOK RETRY CRON]:', e));
}, 5 * 60 * 1000);
...
╚══════════════════════════════════════════════════════╝
  ...
  // Register the Bihr stock sync cron (no-op when BIHR_STOCK_SYNC_DISABLED=true).
